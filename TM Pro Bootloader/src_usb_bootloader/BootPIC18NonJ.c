...
typedef union 
{
		...
		struct{
			...
			unsigned long ProgramMemoryStart;
			unsigned long ProgramMemoryEnd;
	
			unsigned char ExtraPadBytes[49];
		};
...
void _TBLRDPOSTINC(void)
{
	_asm TBLRDPOSTINC _endasm
}

void _TBLWTPOSTINC(void)
{
	_asm tblwtpostinc _endasm
}

void _TBLRDPOSTDEC(void)
{
...
void _GOTO_REMAPPED_RESET_VECTOR_ADDRESS(void)
{
	_asm
		goto REMAPPED_RESET_VECTOR_ADDRESS
	_endasm
}
...
void ProcessIO(void)
{
	static unsigned char isValidated = 0;
	static unsigned char tempNum;
	static unsigned char i;
	idreg_t version;

	if((BootState == BootNotFound) || (BootState == Idle))
	{
	...
	{	
		if(PacketFromPC.Command == CMD_VALIDATE)
		{
			//Encoding
			if(PacketFromPC.RandNum > 128)
				tempNum = (PacketFromPC.RandNum - 84);
			else
				tempNum = (PacketFromPC.RandNum + 101);
			tempNum ^= ENCODED_NUM_XOR_PC;

				//Check
			if(tempNum == PacketFromPC.EncodedNum)
			{
				isValidated = 1; // It's ok, not hacker =)

			    TBLPTR = (unsigned short long)&idreg;
			 
			    for (i = 0; i < 8; i++)
			    {
				   _TBLRDPOSTINC();
			       version.bytes[i] = TABLAT;
			    }
				//Prepare a response packet, which lets the PC software know about the memory ranges of this device.
				PacketToPC.Command  =  CMD_VALIDATE;
				PacketToPC.EncodedNum  = PacketFromPC.RandNum ^ ENCODED_NUM_XOR_BL;
				PacketToPC.MajorVer = version.major_version;
				PacketToPC.MinorVer = version.minor_version;

				PacketToPC.ProgramMemoryStart = (unsigned long)ProgramMemStart;
				PacketToPC.ProgramMemoryEnd   = (unsigned long)ProgramMemStop;
			...
			else  // Wrong PC SOFTWARE, go to main app
			{
				PacketFromPC.Command = CMD_LEAVEUSBMODE; //Leave in the next switch block
			}
		}
...
//					ClrWdt();
			...
			{
				//Check if host is trying to program the USER ID
				if(PacketFromPC.Contents[3] == 0x20) // 			//PacketFromPC.Contents[3] is bits 23:16 of the address.  
				{													//0x20 implies config bits
			...
			case CMD_GET_DATA:
			{
				...
				PacketToPC.Command = CMD_GET_DATA;
...
void WriteConfigBits(void)	//Also used to write the Device ID
{
	...
	for(i = 0; i < PacketFromPC.Size; i++)
	{
		TABLAT = PacketFromPC.Data[i];
		_TBLWT();

		EECON1 = 0b10000100;	//Config bits programming mode
		UnlockAndActivate();
